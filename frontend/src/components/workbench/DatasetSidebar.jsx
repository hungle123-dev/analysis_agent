...
export function DatasetSidebar({ dataset }) {
  ...
  return (
    <aside className="min-w-0 overflow-auto border-r border-white/10 bg-shell p-4 max-[760px]:hidden">
      <div className="mb-4">
      ...
      <Panel title="Schema" meta={`${columns.length} cols`}>
        <div className="grid gap-1.5">
          {columns.map((column) => (
            <div className="grid grid-cols-[minmax(0,1fr)_auto] rounded-md bg-black/20 px-2 py-2" key={column.name}>
      ...
      <Panel title="Workflow" meta="local">
        <div className="grid gap-1.5 text-xs">
          {["Request received", "Proposal generated", "Human review required", "Local execution gated"].map((step) => (
            <div className="rounded-md bg-white/[0.035] px-2 py-1.5 text-muted" key={step}>
              {step}
            </div>
          ))}
        </div>
